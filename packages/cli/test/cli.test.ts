...
test('init: creates SILMARI.md and makes all four agent start files refer to it; records the user language; asks about migration when md files exist', () => {
  ...
  assert.match(sk, /## Notation[\s\S]*\(\(use a subagent\)\)/, 'skeleton is English regardless of language')
  assert.match(sk, /## Migration[\s\S]*One agent, one file[\s\S]*Finish with `sil lint`/, 'skeleton carries the migration rules, not only the notation summary')
