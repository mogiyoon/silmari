...
const en = {
  ...
  print: 'Print', printTitle: 'Print the picture, or the selected node with everything below it: fitted on one A4 page, or at real size across A4 pages (⌘P / Ctrl+P)', printing: 'Preparing to print …', printTooBig: (n: number): string => `Too many nodes to print (${n}). Open one flow and print that`,
  printAll: 'Whole picture', printSub: (title: string): string => `Below the selected node: ${title}`, printNoSel: 'Select a node to print only what is below it', printFit: 'Fit on one page', printTiles: 'Real size · A4 pages', printPage: (r: number, rows: number, c: number, cols: number): string => `row ${r}/${rows} · col ${c}/${cols}`,
...
const ko: Dict = {
  ...
  print: '인쇄', printTitle: '그림 전체 또는 선택한 노드 아래를 인쇄한다. A4 한 장에 맞추거나, 실제 크기로 여러 장에 나눠서 (⌘P / Ctrl+P)', printing: '인쇄 준비 중 …', printTooBig: (n) => `노드 ${n}개는 너무 많아 인쇄할 수 없다. 흐름 하나만 열고 인쇄한다`,
  printAll: '전체', printSub: (title) => `선택한 노드 아래: ${title}`, printNoSel: '노드를 선택하면 그 아래만 인쇄할 수 있다', printFit: '한 장에 맞춤', printTiles: '실제 크기 · A4 여러 장', printPage: (r, rows, c, cols) => `${r}/${rows}행 · ${c}/${cols}열`,
