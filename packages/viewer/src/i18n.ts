...
const en = {
  ...
  subagent: 'subagent', noRules: 'no rules', noRulesOf: (w: string) => `runs without the project start files: {{-${w}}}`, calls: (n: number) => `calls ${n}`,
  dragToMove: 'Drag to move', target: (to: string) => `Target: ${to}`, headingOfCall: 'The heading that contains this call',
  tagSend: 'send', tagReceive: 'receive', tagTools: 'tools', tagModel: 'model',
  ...
  legendTask: 'task', legendDoc: 'reference', legendFile: 'file (not parsed)', legendGhost: 'missing', legendSubagent: 'subagent call (isolated)', legendNoRules: 'runs without the project start files',
...
const ko: Dict = {
  ...
  subagent: '서브 에이전트', noRules: '규칙 제외', noRulesOf: (w) => `프로젝트 시작 파일 없이 실행: {{-${w}}}`, calls: (n) => `호출 ${n}`,
  dragToMove: '끌어서 옮길 수 있다', target: (to) => `대상: ${to}`, headingOfCall: '이 호출이 있는 헤딩',
  tagSend: '보냄', tagReceive: '받음', tagTools: '도구', tagModel: '모델',
  ...
  legendTask: '작업', legendDoc: '자료', legendFile: '파일 (파싱 안 함)', legendGhost: '없음', legendSubagent: '보조 작업 호출(서브에이전트)', legendNoRules: '프로젝트 시작 파일 없이 실행',
  send: '보냄', receive: '받음',
